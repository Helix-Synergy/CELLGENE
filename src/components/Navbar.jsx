...
const Navbar = () => {
  ...
  const nav_links = [
    { name: "Home", link: "/" },
    { name: "About", link: "/about-cellgene" },
    { name: "Executive Panel", link: "/executive-panel-members/" },
    { name: "Tracks", link: "/cellgene-conference-tracks/" },
    { name: "Orators", link: "/cellgene-orators" },
    { name: "Schedule", link: "/cellgene-event-schedule" },
        ...
        >
          {nav_links.map((item, index) => (
            <Link
              key={index}
              to={item.link}
              className={`hover-underline-animation transition ${location.pathname === item.link
                ? "active-nav-link text-one font-semibold"
                : "text-gray-700 hover:text-one"
                }`}
              style={{
                marginRight: scrolled ? "0px" : "4px",
              }}
